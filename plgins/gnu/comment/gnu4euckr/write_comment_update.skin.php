...
if($mb['mb_hp'] && $member['mb_hp'] != $mb['mb_hp'])
{
    ...
    $wr['wr_subject'] = iconv('euc-kr','utf-8',$wr['wr_subject']);
    $member['mb_nick'] = iconv('euc-kr','utf-8',$member['mb_nick']);
    $wr_content = iconv('euc-kr','utf-8',$wr_content);

    $subject = '"'.$wr['wr_subject'].'" 글에 새 댓글이 등록되었습니다.';

    $content = '"'.$wr['wr_subject'].'"글에 '.$member['mb_nick'].'님이 "'.$wr_content.'" 라고 댓글을 달았습니다.';

    $callback_url = $g4[url]/$g4[bbs].'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr['wr_parent'].'&amp;'.$qstr.'&amp;#c_'.$comment_id;
